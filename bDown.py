...
def main():
    if (sys.argv[-1] == '-h'):
        print('Usage:'
              'bDown -a [URL] to download audio file'
              'bDown [URL] to download video file')
    else:
        url=sys.argv[-1]
        ...
        downLoad(audio_link, f"{video_name}.mp3")
        if (sys.argv[1] == '-a'):
            desktop = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop')
            shutil.move(f'{video_name}.mp3', desktop)
        else:
            downLoad(video_link, "video.mp4")
            os.system(f'ffmpeg -i "video.mp4" -i "{video_name}.mp3" -c copy "{video_name}.mp4"')
            desktop = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop')
